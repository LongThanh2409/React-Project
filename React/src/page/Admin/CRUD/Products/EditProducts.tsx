...
const EditProducts = () => {
  ...
  const { register, control, watch, handleSubmit } = useForm<Iproducts>({
    defaultValues: async () => {
      if (id) {
        return await fetchProduct(name!, id);
      }
    },
  });
  ...
  const onSubmit = async (data: Iproducts) => {
    try {
      await editProduct(id!, { ...data, image: uploadUrlsss });
  ...
  const fetchProduct = async (name: string, id: string | number) => {
  ...
  const errorsAlert = () => {
    messageApi.open({
      type: "error",
      content: "Sửa sản phẩm không thành công",
      duration: 3,
