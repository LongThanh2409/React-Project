  ...
  return (
    <>
      <article className="relative rounded-xl bg-white p-3 shadow-xl hover:shadow-2xl hover:transform hover:scale-105 duration-300 border-[#cce7d0]">
        {item.hot_sale === undefined || !item.hot_sale ? null : (
          <div
            className="absolute z-10 -top-1 left-[-3px] py-2 px-2 "
            style={{
          ...
          >
            {" "}
            <span className=" m-2  rounded-full text-center text-sm font-medium text-white">
              {item.hot_sale}% OFF
            </span>
          </div>
        )}
        ...
        <Link
          to={`/detail-product/${encodeURIComponent(item.name)}/${item._id}`}
          className="relative mx-3 mt-3 md:h-60 flex justify-center overflow-hidden rounded-xl"
        >
          <img
            src={item.image[0]}
